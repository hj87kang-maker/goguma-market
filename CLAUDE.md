...
- **폼이 길어서 제출 버튼이 하단 고정 탭바(`BottomNav`, 높이 4rem) 영역과 겹치면, 좌표 기반 클릭이 탭바로 새버릴 수 있음**: `/products/new`에서 이미지 미리보기까지 채운 뒤 스크롤 없이 "등록하기"를 클릭하면 버튼이 스크롤되지 않은 상태로 탭바 뒤에 위치해서(`document.elementFromPoint`로 확인 가능), 클릭이 탭바의 "채팅" 링크로 전달되어 엉뚱하게 `/chat`으로 이동하는 걸 실제로 겪었다. 페이지를 끝까지 스크롤하면(`element.scrollIntoView()`) 정상적으로 버튼이 클릭된다 — 폼 데이터는 유실되지 않고 그대로 남아있음. 재발 방지로 `globals.css`에 `html { scroll-padding-bottom: 4rem; }`를 추가해 스크롤/포커스 시 탭바 높이만큼 여백을 확보하도록 했다. **자동화 테스트로 하단부 버튼을 클릭할 때는 클릭 전에 항상 스크롤(또는 `element.scrollIntoView({ block: 'center' })`)을 먼저 할 것.**
